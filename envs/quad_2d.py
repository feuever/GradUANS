...
class Quad2DEnv(core.Env):
    ...
    def _sample_target(self):
        ...
        x = 2 * self.MAX_X * (self.np_random.random() - 0.5)  # Changed from self.np_random()
        y = 2 * self.MAX_Y * (self.np_random.random() - 0.5)  # Changed from self.np_random()

        return (x, y)

    def _check_target(self, target):
        obstacle_distance = 0.5  # 增加安全距离
        boundary_margin = 0.5   # 边界安全距离

        # 检查是否在边界附近（距离边界小于安全距离）
        if (abs(target[0]) > self.MAX_X - boundary_margin or 
            abs(target[1]) > self.MAX_Y - boundary_margin):
            return False
    ...
    def _sample_target(self):
        # 改进的目标采样方法，确保目标位置在安全区域内
        boundary_margin = 0.5
        obstacle_distance = 0.5
        max_attempts = 100
        
        for attempt in range(max_attempts):
            # Random x/y target position in safe region [-self.MAX_X/Y + margin, self.MAX_X/Y - margin]
            x = np.random.uniform(-self.MAX_X + boundary_margin, self.MAX_X - boundary_margin)
            y = np.random.uniform(-self.MAX_Y + boundary_margin, self.MAX_Y - boundary_margin)
            ...
            if self.n_obstacles > 0:
                for obstacle in self.obstacles:
                    if 'd' in obstacle:  # 矩形障碍物
                        if (abs(x - obstacle['x']) < obstacle['d']/2 + obstacle_distance and 
                            abs(y - obstacle['y']) < obstacle['d']/2 + obstacle_distance):
                            is_valid = False
                            break
                    elif 'r' in obstacle:  # 圆形障碍物
                        distance_to_obstacle = np.sqrt((x - obstacle['x'])**2 + (y - obstacle['y'])**2)
                        if distance_to_obstacle < obstacle['r'] + obstacle_distance:
                            is_valid = False
                            break
        ...
        # 如果无法找到安全位置，返回一个相对安全的位置（中心附近）
        print("警告：无法找到完全安全的目标位置，使用中心位置")
        return (0.0, 0.0)
    ...
    def _generate_obstacles(self):
        ...
        # 创建独立的随机数生成器用于障碍物生成，固定种子为42
        obstacle_rng = np.random.RandomState(42)
    
        counter = 0
        while counter < self.n_moving_obstacles_box:
            d = d_min + obstacle_rng.random() * (d_max - d_min)
            x = (2 * self.MAX_X - d) * (obstacle_rng.random() - 0.5)
            y = (2 * self.MAX_Y - d) * (obstacle_rng.random() - 0.5)
            is_valid = True
            for i in range(counter):
                ...
                if np.linalg.norm(np.array([obstacle['x'], obstacle['y']]) - np.array([x, y])) < dist_min:
                    ...
                    break
            if is_valid:
                vx = self.MAX_VEL_X * (obstacle_rng.random() - 0.5)
                vy = self.MAX_VEL_Y * (obstacle_rng.random() - 0.5)
        ...
        while counter < self.n_moving_obstacles_box + self.n_static_obstacles_box + self.n_moving_obstacles_circle:
            r = r_min + obstacle_rng.random() * (r_max - r_min)
            x = (2 * self.MAX_X - r) * (obstacle_rng.random() - 0.5)
            y = (2 * self.MAX_Y - r) * (obstacle_rng.random() - 0.5)
            is_valid = True
            for i in range(counter):
                obstacle = self.obstacles[i]
                if np.linalg.norm(np.array([obstacle['x'], obstacle['y']]) - np.array([x, y])) < dist_min:
