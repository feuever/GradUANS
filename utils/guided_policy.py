...
def n_step_doubleguided_p_sample(
    model, x, cond, t, guide, scale=0.001, t_stopgrad=2, n_guide_steps=2, scale_grad_by_std=True, scale_cbf=5.0, scale_uncertainty=1.0): #0.01 #2 #4
    ...
    for _ in range(n_guide_steps):
        with torch.enable_grad():
            ...
            grad_uncertainty = torch.autograd.grad([uncertainty.sum()], [x])[0]
            x.detach()

        if scale_grad_by_std:
            grad0 = model_var * grad0
            grad1 = model_var * grad1
            grad_uncertainty = model_var * grad_uncertainty

        grad0[t < t_stopgrad] = 0
        grad1[t < t_stopgrad] = 0
        grad_uncertainty[t < t_stopgrad] = 0

        x = x + scale * (grad0) + scale_cbf * (grad1) - scale_uncertainty * grad_uncertainty
        x = apply_conditioning(x, cond, model.action_dim)
